            ...
            {categoriasUnicas.map((cat) => (
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded-lg font-bold transition-all ${
                  categoriaSeleccionada === cat ? 'bg-[#5e5345] text-white' : 'text-[#5e5345] hover:bg-[#dccab0]'
                }`}
              >
                {cat.toUpperCase()}
              </button>
        ...
        <FilterBox titulo="Precio">
          <div className="flex flex-col gap-3">
            <input 
              type="number" 
              placeholder="Desde ($)" 
              className="w-full p-2 rounded-lg border border-[#dccab0] bg-white text-[#5e5345]"
      ...
      <section className="flex-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {productosFiltrados.map((prod) => (
            <ProductCard key={prod.id} {...prod} cantidadFotos={2} />
          ))}
        </div>
      </section>
    </main>
